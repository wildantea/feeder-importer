                ...
                <section class="content-header">
                    <h1>
                        Manage Krs <?php echo $db->fetch_single_row('jurusan','kode_jurusan',$id_jur)->nama_jurusan;?>
                    </h1>
                        <ol class="breadcrumb">
                        <li><a href="<?=base_index();?>"><i class="fa fa-dashboard"></i> Home</a></li>
                        <li><a href="<?=base_index();?>krs">Krs</a></li>
                        <li class="active">Krs List</li>
                    </ol>
                </section>
       ...
       foreach ($db->fetch_all("sys_menu") as $isi) {
          ...
          <a href="<?=base_index();?>krs/import/<?=$id_jur;?>" class="btn btn-primary btn-flat"><i class="fa fa-cloud-upload"></i> Upload Data Excel</a>
...
<div class="form-group">
               ...
               <?php foreach ($db->fetch_custom("select krs.semester,nama_semester from krs inner join semester on
 krs.semester=semester.semester where kode_jurusan='".$id_jur."' group by krs.semester order by krs.semester desc") as $isi) {
...
<div class="form-group">
            ...
            <!-- /.box-body -->
                                    ...
                                    <table id="dtb_krs" class="table table-bordered table-striped">
                                   <thead>
                                     <tr>
...
<div class="row">
        <div class="col-lg-12">
            <div class="pull-right">
                ...
                <button type="button" data-id="<?=$id_jur;?>" data-uri="<?=base_admin();?>modul/krs/krs_action.php"  class="btn btn-danger" id="hapus_data_error"><i class="fa fa-info-circle"></i> hapus data error</button>
                <button type="button" data-id="<?=$id_jur;?>" data-uri="<?=base_admin();?>modul/krs/krs_action.php" class="btn btn-danger" id="hapus_massal"><i class="fa fa-recycle"></i> Hapus Data Massal</button>
            </div>
        </div>
        <!-- /.col-lg-12 -->
    </div>
...
<?php foreach ($db->fetch_custom("select krs.semester,nama_semester from krs inner join semester on
 krs.semester=semester.semester where kode_jurusan='".$id_jur."' group by krs.semester") as $isi) {
 echo "<option value='$isi->semester'>$isi->semester $isi->nama_semester</option>";
               } ?>
              </select>
                        </div>
 ...
 <button type="button" id="delete" class="btn btn-danger">Delete</button> <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button> </div> </div><!-- /.modal-content --> </div><!-- /.modal-dialog --> </div><!-- /.modal -->
       ...
       foreach ($db->fetch_all("sys_menu") as $isi) {
                      if ($path_url==$isi->url) {
                          if ($role_act["insert_act"]=="Y") {
                    ?>
          <a href="<?=base_index();?>krs/tambah/<?=$id_jur;?>" class="btn btn-primary btn-flat"><i class="fa fa-plus"></i> Tambah</a>
                          <?php
                          }
                       }
}
  ...
  if ($role_act['del_act']=='Y') {
   $del = "<span data-id='+aData[indek]+' data-uri=".base_admin()."modul/krs/krs_action.php".' class="btn btn-danger hapus btn-flat"><i class="fa fa-trash"></i></span>';
  } else {
    $del="";
  }
                   }
...
var dataTable = $("#dtb_krs").dataTable({
     ...
     $('td:eq('+indek+')', nRow).html('<?=$edit;?> <?=$del;?>');
       $(nRow).attr('id', 'line_'+aData[indek]);
        ...
        "columnDefs": [ {
            ...
            "ajax":{
              url :"<?=base_admin();?>modul/krs/krs_data.php",
...
$(function(){
    ...
    $.ajax({
     url: '<?=base_admin();?>modul/krs/create_json.php?jurusan='+<?=$id_jur;?>,
      });
...
$('#sem_filter').on('change', function() {
    $(this).valid();
      ...
      $.ajax({
          url : "<?=base_admin();?>modul/krs/isi_matkul.php",
          type : "post",
...
$('#filter_matkul').on('change', function() {
    $(this).valid();
      $("#filter_kelas").chosen();
      $.ajax({
          url : "<?=base_admin();?>modul/krs/isi_kelas.php",
          type : "post",
...
$('#matkul').on('change', function() {
  ...
  $("#isi_drop_up").hide();
      ...
      $(".text-wait-up").show();
        ...
        window.finished = false;
        $.getJSON('<?=base_admin();?>modul/krs/push_krs.php?sem='+$("#semester_up").val()+"&jurusan="+<?=$id_jur;?>+"&matkul="+$("#matkul").val()+"&kelas="+$("#pilih_kelas").val(),
            function(data){
...
$('.up_feeder').on('click', function() {
 ...
 $('.up_feeder_nows').click(function(){
      ...
      $(".text-wait-up").show();
        ...
        window.finished = false;
        $.getJSON('<?=base_admin();?>modul/krs/push_krs.php?sem='+$("#semester_up").val()+"&jurusan="+<?=$id_jur;?>,
            function(data){
                console.log("ALL DONE", data);
                clearInterval(window.progressInterval);
                window.finished = true;
...
function checkProgress(createStatusBars){
    ...
    url = "<?=base_admin();?>modul/krs/<?=$id_jur;?>_progress.json";
