       ...
       foreach ($db->fetch_all("sys_menu") as $isi) {
                    ...
                    ?>
          <a href="<?=base_index();?>kelas-kuliah/import/<?=$id_jur;?>" class="btn btn-primary btn-flat"><i class="fa fa-cloud-upload"></i> Upload Data Excel</a>
...
<div class="box box-danger">
 ...
 kelas_kuliah.semester=semester.semester where kode_jurusan='".$id_jur."' group by kelas_kuliah.semester order by kelas_kuliah.semester desc") as $isi) {
                  echo "<option value='$isi->semester'>$isi->semester $isi->nama_semester</option>";
                        ...
                        <div class="col-lg-3">
                        <select id="filter_matkul" data-placeholder="Pilih Semester ..." class="form-control chzn-select" tabindex="2">
                        <option value="all">Semua</option>
              </select>
...
<div class="form-group">
            ...
            <!-- /.box-body -->
                                    ...
                                    <table id="dtb_kelas_kuliah" class="table table-bordered table-striped">
                                   <thead>
                                     <tr>
   <th><input type="checkbox"  id="bulkDelete"  /> <button id="deleteTriger"><i class="fa fa-trash"></i></button></th>
        ...
        <div class="col-lg-12">
            <div class="pull-right">
                ...
                <button type="button" data-id="<?=$id_jur;?>" data-uri="<?=base_admin();?>modul/kelas_kuliah/kelas_kuliah_action.php"  class="btn btn-danger" id="hapus_data_error"><i class="fa fa-info-circle"></i> hapus data error</button>
                <button type="button" data-id="<?=$id_jur;?>" data-uri="<?=base_admin();?>modul/kelas_kuliah/kelas_kuliah_action.php" class="btn btn-danger" id="hapus_massal"><i class="fa fa-recycle"></i> Hapus Data Massal</button>
            </div>
        </div>
        <!-- /.col-lg-12 -->
    </div>
...
<input type="hidden" id="semester_selected">
  ...
  $edit = '<a href="'.base_index()."kelas-kuliah/edit/'+aData[indek]+'/$id_jur".'" class="btn btn-primary btn-flat"><i class="fa fa-pencil"></i></a>';
  } else {
    $edit ="";
  }
  if ($role_act['del_act']=='Y') {
   $del = "<span data-id='+aData[indek]+' data-uri=".base_admin()."modul/kelas_kuliah/kelas_kuliah_action.php".' class="btn btn-danger hapus btn-flat"><i class="fa fa-trash"></i></span>';
  } else {
    $del="";
  }
                   } 
...
$('#semester_up').on('change', function() {
   ...
   $.ajax({
     url: '<?=base_admin();?>modul/kelas_kuliah/create_json.php?jurusan='+<?=$id_jur;?>,
      });
...
$('#matkul').on('change', function() {
    $(this).valid();
      $("#pilih_kelas").chosen();
      $.ajax({
          url : "<?=base_admin();?>modul/kelas_kuliah/isi_kelas.php",
          type : "post",
...
var dataTable = $("#dtb_kelas_kuliah").dataTable({
     ...
     $('td:eq('+indek+')', nRow).html('<?=$edit;?> <?=$del;?>');
       $(nRow).attr('id', 'line_'+aData[indek]);
           ...
           'bProcessing': true,
            ...
            "ajax":{
              url :"<?=base_admin();?>modul/kelas_kuliah/kelas_kuliah_data.php",
...
$('#deleteTriger').on("click", function(event){
 ...
 $("#loadnya").show();
          $.ajax({
              type: "POST",
              url: "<?=base_admin();?>modul/kelas_kuliah/kelas_kuliah_action.php?act=del_massal",
...
$('.up_feeder').on('click', function() {
  ...
  $("#isi_drop_up").hide();
      ...
      $(".text-wait-up").show();
        ...
        window.finished = false;
        $.getJSON('<?=base_admin();?>modul/kelas_kuliah/push_kelas.php?sem='+$("#semester_up").val()+"&jurusan="+<?=$id_jur;?>+"&matkul="+$("#matkul").val()+"&kelas="+$("#pilih_kelas").val(),
            function(data){
