...
//$token = 'acdbbc82c3b29f99e9096dab1d5eafb4';
	...
	if ($jumlah>=500) {
		...
		if ($i==0) {
			...
			$data = $db->fetch_custom("select *,nilai.id as id_krs from nilai inner join jurusan on nilai.kode_jurusan=jurusan.kode_jurusan where jurusan.kode_jurusan='".$_GET['jurusan']."' and nilai.semester='".$_GET['sem']."' and status_error!=1 $query_matkul $query_kelas limit $i,500");
		...
		if ($id_reg_pd!="" && $error_krs=="" && $id_kls!="") {
			...
			if ($check_krs['result']) {
	 		...
	 		$data = $db->fetch_custom("select *,nilai.id as id_krs from nilai inner join jurusan on nilai.kode_jurusan=jurusan.kode_jurusan where jurusan.kode_jurusan='".$_GET['jurusan']."' and nilai.semester='".$_GET['sem']."' and status_error!=1 $query_matkul $query_kelas limit ".($i*500).",$bagi");
			//let's push first page
			$stageOptions = array(
			    'name' => 'Page $i',
			    'message' => 'Some Message',
		...
		if ($id_reg_pd!="" && $error_krs=="" && $id_kls!="") {
        	...
        	$total_record = ($i*500+$bagi);
	 		...
	 		$data = $db->fetch_custom("select *,nilai.id as id_krs from nilai inner join jurusan on nilai.kode_jurusan=jurusan.kode_jurusan where jurusan.kode_jurusan='".$_GET['jurusan']."' and nilai.semester='".$_GET['sem']."' and status_error!=1 $query_matkul $query_kelas limit ".($i*500).",500");
			//let's push first page
			$stageOptions = array(
			    'name' => 'Page $i',
			    'message' => 'Some Message',
