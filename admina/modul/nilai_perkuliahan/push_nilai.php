...
$token = get_token();
	...
	$error = array();
		...
		$pu = new Manticorp\ProgressUpdater($options);
			$data = $db->query("select *,nilai.id as id_krs from nilai inner join jurusan on nilai.kode_jurusan=jurusan.kode_jurusan where jurusan.kode_jurusan='".$_GET['jurusan']."' and nilai.semester='".$_GET['sem']."' and status_error!=1 $query_matkul $query_kelas");
			...
			$temp_kls = service_request($data_req_kelas);
				if (!empty($temp_kls->data)) {
					...
					if ($temp_result->error_desc=='') {
						...
						$db->update('nilai',array('status_error' => 2, 'keterangan'=> $temp_result->error_desc),'id',$value->id_krs);
					}
				} else {
					++$error_count;
					$error_msg[] = $temp_result->error_desc;
					$db->update('nilai',array('status_error' => 2, 'keterangan'=> 'Pastikan Kelas Sudah Dibuat'),'id',$value->id_krs);
				}
			} else {
				++$error_count;
				$error_msg[] = 'Mahasiswa ini tidak ada di feeder';
				$db->update('nilai',array('status_error' => 2, 'keterangan'=> 'Mahasiswa ini tidak ada di feeder'),'id',$value->id_krs);
			}
