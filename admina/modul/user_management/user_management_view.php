...
<th>Aktif</th>
      ...
      $dtb=$pg->myquery("select nama_jurusan,sys_users.aktif,sys_users.username,sys_users.email,sys_group_users.level,sys_users.id,nama_jurusan from sys_users  inner join sys_group_users on sys_users.id_group=sys_group_users.id left join jurusan on sys_users.kode_jurusan=jurusan.kode_jurusan $search",$limit);
       $no=$pg->Num($limit);
        $count=$pg->Num($limit);
      if ($dtb->rowCount()<1) {
